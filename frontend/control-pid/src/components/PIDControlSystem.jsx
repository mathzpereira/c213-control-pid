...
export default function PIDControlSystem() {
  ...
  const [modelParameters, setModelParameters] = useState({ k: 0, tau: 0, theta: 0 });
  const [pidParameters, setPidParameters] = useState({ kp: 0, ti: 0, td: 0 });
  const [customPid, setCustomPid] = useState({ kp: 1, ti: 1, td: 0.1, setpoint: 1 });
  ...
  const handleIdentifyModel = async () => {
    ...
    try {
      ...
      alert('Error identifying model. Please check your connection and try again.');
    } finally {
  ...
  const handleCompareResponse = async () => {
    ...
    try {
      ...
      alert('Error fetching comparison. Please check your connection and try again.');
    } finally {
  ...
  const handleOpenLoopResponse = async () => {
    ...
    try {
      ...
      alert('Error fetching open loop response. Please check your connection and try again.');
    } finally {
  ...
  const handleClosedLoopResponse = async () => {
    ...
    try {
      ...
      alert('Error fetching closed loop response. Please check your connection and try again.');
    } finally {
  ...
  const handleCustomPidSimulation = async () => {
    ...
    try {
      ...
      alert('Error in custom PID simulation. Please check your connection and try again.');
    } finally {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="flex flex-grow overflow-hidden">
        ...
        <div className="w-80 bg-white p-4 shadow-md overflow-auto">
          <div className="mb-4">
            ...
            <input 
              ...
              className="block w-full text-sm text-white mb-2 bg-blue-400 cursor-pointer py-2 px-3 border rounded"
            />
            <button 
          ...
          <div className="mb-4">
            <h2 className="font-semibold text-lg mb-2">Identificação do Modelo</h2>
            <select 
        ...
        <div className="flex-1 p-4 overflow-auto">
          ...
          <div className="flex mb-4 border-b">
            <button 
              className={`px-4 py-2 ${activeTab === 1 ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
              onClick={() => setActiveTab(1)}
            >
              Comparar Resposta
            </button>
            <button 
              className={`px-4 py-2 ${activeTab === 2 ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
            ...
            <button 
              className={`px-4 py-2 ${activeTab === 3 ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
            ...
            <button 
              className={`px-4 py-2 ${activeTab === 4 ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
          ...
          <div className="bg-white p-4 rounded shadow-md h-full">
            ...
            {activeTab === 1 && (
              <div className="h-full">
                <h2 className="text-xl font-semibold mb-4">Comparação de Resposta: Real vs Modelo</h2>
                {compareData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="80%">
                    <LineChart data={compareData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="real" name="Resposta Real" stroke="#8884d8" dot={false} />
                      <Line type="monotone" dataKey="model" name="Modelo Simulado" stroke="#82ca9d" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex items-center justify-center h-full text-gray-500">
                    {loading.compare ? 'Carregando dados...' : 'Identifique o modelo para visualizar a comparação'}
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 2 && (
              <div className="h-full">
                <h2 className="text-xl font-semibold mb-4">Resposta em Malha Aberta</h2>
                {openLoopData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="80%">
                    <LineChart data={openLoopData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="response" name="Malha Aberta" stroke="#ff7300" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex items-center justify-center h-full text-gray-500">
                    {loading.openLoop ? 'Carregando dados...' : 'Sintonize o PID para visualizar a resposta em malha aberta'}
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 3 && (
              <div className="h-full">
                <h2 className="text-xl font-semibold mb-4">Resposta em Malha Fechada</h2>
                {closedLoopData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="80%">
                    <LineChart data={closedLoopData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="response" name="Malha Fechada" stroke="#2196F3" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex items-center justify-center h-full text-gray-500">
                    {loading.closedLoop ? 'Carregando dados...' : 'Sintonize o PID para visualizar a resposta em malha fechada'}
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 4 && (
              <div className="h-full">
                <h2 className="text-xl font-semibold mb-4">Simulação PID Customizado</h2>
                {customPidData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="80%">
                    <LineChart data={customPidData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="setpoint" name="Setpoint" stroke="#FF5722" dot={false} strokeDasharray="5 5" />
                      <Line type="monotone" dataKey="output" name="Saída" stroke="#4CAF50" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex items-center justify-center h-full text-gray-500">
                    {loading.customPid ? 'Carregando dados...' : 'Configure e simule para visualizar a resposta do PID customizado'}
                  </div>
                )}
              </div>
            )}
      ...
      <footer className="bg-gray-200 text-center p-2 text-sm text-gray-600">
        Sistema de Controle PID - C213 Sistemas Embarcados - © <a href="https://github.com/mathzpereira" className="text-gray-600 hover:underline">Matheus Pereira</a>
      </footer>
    </div>
  );
}